...
class Differential {
  public:
    Differential();
    template<typename T>
    static T FirstDerivative(std::function<T(T)> f, T x, T dx);
    template<typename T>
    static T SecondDerivative(std::function<T(T)> f, T x, T dx);
    template<typename T>
    static std::vector<T> Gradient(std::function<T(std::vector<T>)> f,
                                   std::vector<T> Xv,
                                   T dx);
    template<typename T>
    static marsvin::Matrix<T> Jacobian(
        std::vector<std::function<T(std::vector<T>)>> F,
        std::vector<T> X,
        T dx);
};
}  // namespace marsvin
...
template<typename T>
T marsvin::Differential::FirstDerivative(std::function<T(T)> f, T x, T dx) {
    // f'(x) =
    T df;
...
template<typename T>
T marsvin::Differential::SecondDerivative(std::function<T(T)> f, T x, T dx) {
...
template<typename T>
std::vector<T> marsvin::Differential::Gradient(
...
template<typename T>
marsvin::Matrix<T> marsvin::Differential::Jacobian(
    std::vector<std::function<T(std::vector<T>)>> F,
    std::vector<T> Xv,
    T dx) {
